...
class Edrone():
	...
	def __init__(self):
		...
		self.setpoint = [0,0,0,0] # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly
		...
		self.Kp = [6.3,9.7,6.20,0]
		self.Ki = [0,0,0,0]
		self.Kd = [1.9,2.9,30.1,0]
		...
		self.need_pub=rospy.Publisher('/path/need',Int16,queue_size=1)		
		...
		rospy.Subscriber('/vrep/waypoints',PoseArray,self.next_point)
		...
		rospy.Subscriber('/vrep/waypoints', PoseArray, self.path_func)	
	...
	def next_point(self,point):
		self.setpoint[0]=point.poses[0].position[x]
   		self.setpoint[1]=point.poses[0].position[y]
   		self.setpoint[2]=point.poses[0].position[z]
	...
	#	3. Calculate the pid output required for each axis. For eg: calcuate self.out_roll, self.out_pitch, etc.
		#out_pitch
			...
			i_pitch=error_pitch+i_pitch
			self.out_pitch=(self.Kp[0]*error_pitch)+(self.Ki[0]*i_pitch)+(self.Kd[0]*(error_pitch-d_pitch))
			d_pitch=error_pitch
		...
		#out_roll
			...
			i_roll=error_roll+i_roll		
			self.out_roll=(self.Kp[1]*error_roll)+(self.Ki[1]*i_roll)+(self.Kd[1]*(error_roll-d_roll))
			d_roll=error_roll
		...
		#out_throttle
			...
			i_throt=error_throttle+i_throt
			self.out_throttle=(self.Kp[2]*error_throttle)+(self.Ki[2]*i_throt)+(self.Kd[2]*(error_throttle-d_throt))
		...
		#out_yaw
			...
			i_yaw=error_yaw+i_yaw		
			self.out_yaw=self.Kp[3]*error_yaw+self.Ki[3]*i_yaw+self.Kd[3]*(error_yaw-d_yaw)
			d_yaw=error_yaw
